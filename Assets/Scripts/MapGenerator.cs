...
public class MapGenerator : MonoBehaviour
{
    ...
    Queue<Coord> shuffledOpenTileCoords; // 记录没有障碍物的随机Coord队列
    Transform[,] tileMap; // 记录每个地砖的坐标 （(0,0)开始）
    ...
    public void GenerateMap()
    {
        ...
        List<Coord> allOpenCoords = new List<Coord>(allTileCoords); // 先获取所有地砖，然后将生成障碍物的地砖删除
        ...
        for (int i = 0; i < obstacleCount; i++)
        {
            ...
            if (randomCoord != currentMap.mapCenter && MapIsFullyAccessible(obstacleMap, currentObstacleCount))
            {
                ...
                allOpenCoords.Remove(randomCoord); // 删除有障碍物的地砖
            } 
            else
            {
    ...
    // 当玩家长时间静止时，在玩家位置（附近）生成敌人
    public Transform GetTileFromPosition(Vector3 position)
    {
        int x = Mathf.RoundToInt(position.x / tileSize + (currentMap.mapSize.x - 1) / 2f); // 根据上面的CoordTOPosition反推
        int y = Mathf.RoundToInt(position.z / tileSize + (currentMap.mapSize.y - 1) / 2f); // 注意这里是position.z
        //防止越界
    ...
    public Coord GetRandomCoord()
    {
        ...
        shuffledTileCoords.Enqueue(randomCoord); // 将其返回至队尾（这里可能看不出来有啥作用），在GetRandomOpenTile里有用
        return randomCoord;
    }
    ...
    public Transform GetRandomOpenTile()
    {
        ...
        shuffledOpenTileCoords.Enqueue(randomCoord); // 敌人数量过多的话，这样可以解决地砖数量不够的问题
